...
def datas_de_jogo(dados):
    return list(dados['fases']['2700']['jogos']['data'])
...
def busca_imprecisa_por_nome_de_time(dados,nome_time):
    timin= []
    for x in dados['equipes']:
        if nome_time in dados['equipes'][x]['nome-comum'] or nome_time in dados['equipes'][x]['nome-slug'] or nome_time in dados['equipes'][x]['sigla'] or nome_time in dados['equipes'][x]['nome']:
            timin.append(x)
    return timin
...
def ids_de_jogos_de_um_time(dados,time_id):
    ...
    for x in dados['fases']['2700']['jogos']['id']:
        if dados['fases']['2700']['jogos']['id'][x]["time1"] == time_id or dados['fases']['2700']['jogos']['id'][x]["time2"] == time_id:
            timin_jogo.append(x)
    return timin_jogo
...
def datas_de_jogos_de_um_time(dados,nome_time):
    ...
    timin_jogo=[]
    for x in dados['fases']['2700']['jogos']['id']:
        if dados['fases']['2700']['jogos']['id'][x]["time1"] == time or dados['fases']['2700']['jogos']['id'][x]["time2"] == time:
            datin = dados['fases']['2700']['jogos']['id'][x]['data']
            timin_jogo.append(datin)
    return timin_jogo
...
def dicionario_de_gols(dados):
    ...
    for x in dados["equipes"]:
        gols[x]=0
        for y in dados['fases']['2700']['jogos']['id']:
            if dados['fases']['2700']['jogos']['id'][y]["time1"] == x:
                gols[x] += int(dados['fases']['2700']['jogos']['id'][y]["placar1"])
            elif dados['fases']['2700']['jogos']['id'][y]["time2"] == x:
                gols[x] += int(dados['fases']['2700']['jogos']['id'][y]["placar2"])
    return gols
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    jogos_estadio={}
    for x in dados['fases']['2700']['jogos']['id']:
        for y in dados['fases']['2700']['jogos']['id'][x]:
            if y == 'estadio_id':
                h= dados['fases']['2700']['jogos']['id'][x]['estadio_id']
                if h not in estadio:
                    estadio.append(h)
                    jogos_estadio[h]=0
    for z in dados['fases']['2700']['jogos']['id']:
        jogos_estadio[dados['fases']['2700']['jogos']['id'][z]['estadio_id']]+=1       
    return jogos_estadio
...
def qtos_libertadores(dados):
    a = list(dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa'])
    return int(a[2])+1-int(a[0])
...
def classificados_libertadores(dados):
    a = list(dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa'])
    lista=[]
    for x in range(0, int(a[2])+1-int(a[0])):
        lista.append(dados["fases"]["2700"]["classificacao"]["grupo"]["Único"][x])
    return lista
...
def rebaixados(dados):
    a = list(dados['fases']['2700']['faixas-classificacao']['classifica3']['faixa'])
    lista=[]
    for x in range(int(a[0]+ a[1])-1,int(a[3]+a[4])):
        lista.append(dados["fases"]["2700"]["classificacao"]["grupo"]["Único"][x])
    return lista
...
def classificacao_do_time_por_id(dados,time_id):
    y = dados["fases"]["2700"]["classificacao"]["grupo"]["Único"]
    for x in y:
        if time_id == x:
            return y.index(x)+1
    return 'nao encontrado'
